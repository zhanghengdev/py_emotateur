...
class emotateur():
    def __init__(self):
        ...
        self.ui.next_button.clicked.connect(self.load_next_reference_image)
    ...
    def start_home_screen(self):
        ...
        self.ui.set_ui_mode(0)
        self.ui.update_score(0)
        self.detected = False

    def load_next_reference_image(self):
        current_image = self.get_current_image()
        img_reference_file_name = os.path.join('img','{}.jpg'.format(current_image))
        ...
        faceBB = self.reference_faceBB[current_image]
        ...
        self.ui.update_left_label_down_with_pixmap(opencvimg_2_pixmap(res_reference))
        #cv2.imwrite('{}.png'.format(current_image),cv2.cvtColor(res_reference, cv2.COLOR_BGR2RGB))
    ...
    def start_one_player_game(self):
        ...
        self.start_1s_block_time = time.time()    #block 1s bfore compare
        self.load_next_reference_image()
        self.one_player_game_timer.start(1000/2)
        self.ui.set_ui_mode(1)
    ...
    def start_two_players_game(self):
        ...
        self.ui.set_ui_mode(2)
        self.result_win = result_win(QtWidgets.QWidget())

    def two_players_mode_update_reference(self):
        time_used = time.time()-self.start_time
        self.ui.update_time_left(12-time_used)
        if time_used > 12:
            self.two_players_left_timer.stop()
            self.start_time = time.time()
            self.faceBB = [150, 75, 300, 300]
        ...
        self.reference_image = opencvimg_2_pixmap(frame)
        self.ui.update_left_label_up_with_pixmap(self.reference_image)
        self.ui.update_left_label_down_with_pixmap(opencvimg_2_pixmap(res))

    def two_players_mode_update_score(self):
        time_used = time.time()-self.start_time
        self.ui.update_time_left(30-time_used)
        if time_used > 30:
            self.two_players_right_timer.stop()
            self.start_time = time.time()
            self.faceBB = [150, 75, 300, 300]
        ...
        frame = self.draw_rectangle(frame)
        res = self.draw_rectangle(res)
        frame = opencvimg_2_pixmap(frame)
        self.ui.update_right_label_up_with_pixmap(frame)
        self.ui.update_right_label_down_with_pixmap(opencvimg_2_pixmap(res))
    ...
    def one_player_mode_update_frame(self):
        frame = self.read_image_from_webcam()
        frame = self.draw_rectangle(frame)
        self.ui.update_right_label_up_with_pixmap(opencvimg_2_pixmap(frame))
    ...
    def one_player_mode_update_score(self):
        ...
        self.ui.update_right_label_down_with_pixmap(opencvimg_2_pixmap(res))
        similarity = self.fc.compare_face(self.face_key_points_reference, face_key_points)
        similarity = similarity if time.time()-self.start_1s_block_time > 1 else 0
        self.ui.update_score(similarity)
        time_used = time.time()-self.start_time
        ...
        if time_used > 60:
            ...
            self.start_home_screen()
        if similarity > 90:
            self.result_win.add_images(self.reference_image,opencvimg_2_pixmap(frame))
            self.load_next_reference_image()
            self.start_1s_block_time = time.time()
